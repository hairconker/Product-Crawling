...
本仓库刻意保留三套实现共存，不要误删其中任何一套——每套对应不同的"降级场景"：
...
- **`run_jd_drission.py`** —— DrissionPage 版，**仅 JD**。当 Playwright 指纹被京东识破（机房 IP / stealth JS 失效）时的兜底路径：通过 `--remote-debugging-port` 接管用户手动登录的真实 Chrome 进程。和前两个脚本**不相互依赖**，CSV 输出格式刻意与 `viewer.html` 兼容。
- **`core/` 目录是"未来版"脚手架**，用 `loguru / pydantic / tenacity / fastapi` 等第三方库，依赖 `pip install -r requirements.txt` 才能跑。当前 `run_cpu_crawl*.py` / `run_jd_drission.py` **不**从 `core/` import 任何东西（自包含），但 `core/` 定义的异常体系和模型是全项目契约，新代码应以其为准。
...
# 批量关键词（新）：--keywords 或 --keywords-file 二选一，模板见 keywords.example.txt
python run_cpu_crawl_pw.py --keywords "i5-12400F,R5 7500F" --pages 2
python run_cpu_crawl_pw.py --keywords-file keywords.txt --pages 2 --headed

# 最小依赖版（仅 JD 有效）
python run_cpu_crawl.py "i5-12400F" --only jd

# JD 兜底（Playwright 被风控后走 DrissionPage 接管真实 Chrome）
#   前置：chrome.exe --remote-debugging-port=9222 --user-data-dir="E:\chrome_jd_crawler"，手动扫码登录
python run_jd_drission.py "i5-12400F" --port 9222
python run_jd_drission.py --keywords "a,b,c" --pages 2

# 查看结果：浏览器直接打开 viewer.html，拖入 logs/ 下的 CSV / JSON 即可

# 编译自检
python -m py_compile run_cpu_crawl_pw.py run_cpu_crawl.py run_jd_drission.py scripts\login_helper.py
```
...
`run_cpu_crawl_pw.py` 顶部定义两类节流：

1. **逐步等待**（请求级，已尽量调小）：`SCROLL_DELAY_MS=1500`、`PAGE_DELAY_MS=2000`、`PLATFORM_DELAY_SEC=5.0`、`JD_PAGE_DELAY_MS=3000`、`CAPTCHA_WAIT_SEC=300`（不动）。
2. **成果批次节流**（产出级，主限速器）：`BATCH_SIZE=30` + `BATCH_REST_SEC=45`。全局累计抓到 30 条触发一次 45s 长休（`_maybe_batch_rest` 在 `run_platform_batch` 每关键词完成后调用，跨平台跨关键词共享 `_BATCH_COUNTER`，每次 `main()` / `_run_dict_mode()` 入口 `_reset_batch_counter()`）。

**`--fast` 激进档**：CLI 加 `--fast` 触发 `_apply_fast_preset()`，覆盖 `BATCH_SIZE→100 / BATCH_REST_SEC→15 / KEYWORD_DELAY_SEC→5 / KEYWORD_BATCH_SIZE→8 / PLATFORM_DELAY_SEC→2`。`JD_*` 常量刻意保留（JD 风控最敏感，不波及）。预设字典 `_FAST_PRESET` 在文件顶部。理论提速 3-4 倍，但淘宝命中 `rgv587_flag` 概率显著上升，第一次跑务必盯 `logs/debug/taobao_*.png`。

设计意图：原先逐页/跨平台硬等待让批量任务非常慢；改为按"累计成果数"长休更接近"人类浏览—翻页—暂停"的节奏，在不显著放大风控暴露的前提下提速。**业务代码一律引用常量**，不要散写数字，也别随手调回旧值——风控策略变化时改这里。淘宝滑块时 `_wait_for_captcha_pass()` 会等用户手动拖（仅 `--headed` 模式有效）。
...
- **`viewer.html`** —— 纯前端（无后端）的结果查看器，浏览器直接打开，拖入 `logs/` 下的 CSV 或 JSON 即可。`run_jd_drission.py` 的输出字段**必须**与之保持兼容（改列名前确认 viewer 能解析）。
- **`config/config.yaml.example`** —— 配置模板（未来 `core/` 版启用后使用）。当前脚本仍靠 CLI 参数，不读它。
...
4. `run_jd_drission.py` 迁移时保留为独立文件（如 `spiders/jd_drission.py`），**不要**合并进 `spiders/jd.py`——它是指纹检测兜底分支，合并会牺牲回退能力
5. 按阶段 5 文档起 FastAPI 服务

在那之前，**`run_cpu_crawl_pw.py` 是主生产入口，`run_jd_drission.py` 是 JD 兜底**，修改时保持自包含（别 `from core import ...`）。
